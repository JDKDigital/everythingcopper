...
{
    public CopperMinecartHopper(EntityType<? extends MinecartHopper> type, Level level) {
        super(type, level);
    }

    public CopperMinecartHopper(Level level, double x, double y, double z) {
        super(level, x, y, z);
    }
    ...
    protected Component getTypeName() {
        String id = (isWaxed() ? "waxed_" : "") + getAge().name().toLowerCase() + "_hopper_copper_minecart";
        return new TranslatableComponent(this.getType().getDescriptionId().replace("hopper_copper_minecart", id));
    }
